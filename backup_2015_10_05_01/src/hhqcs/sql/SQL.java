    ...
    public void record(CoilHeader ch, byte[] record, Setup setup) {
        ...
        try {
            ...
            String log = "INSERT INTO " + setup.PLANTTABLENAME + " (tekercsszam, idobelyeg, tekercshossz, adat)VALUES ("
                    + coilIdString + "," + ch.timeStamp + "," + ch.coilLength + ",BLOB)";
            ...
            debug.printDebugMsg(setup.PLANTNAME, this.getClass().getCanonicalName(), "INSERT INTO " + setup.PLANTTABLENAME + " (tekercsszam, idobelyeg, tekercshossz, adat)VALUES ("
                    + coilIdString + "," + ch.timeStamp + "," + ch.coilLength + ",BLOB)", ex);
        } finally {
            closeConnection(con, st);
        }
    ...
    public void thicknessRecord(ThicknessHeader thicknessHeader, byte[] record, Setup setup) {
        ...
        try {
            if (record != null && record.length > 0) {
                ...
                System.out.println(new Date().toString() + " " + setup.PLANTNAME + " - " + "No record to write blob");
                debug.printDebugMsg(setup.PLANTNAME, this.getClass().getCanonicalName(), "(error) No record to write blob)");
            }
        } catch (SQLException ex) {
            System.err.println(ex);
