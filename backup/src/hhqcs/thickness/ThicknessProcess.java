...
public class ThicknessProcess {
    ...
    private static final int DELETEDTHICKNESSLENGTH = 8;
    ...
    public void process(TCPConnectionServer tcp) {
        ...
        if (tcp.receiveTelegram.length == TELEGRAMLENGTH) {
            ...
            if (!thicknessHeader.coilID.equals(previousThicknessHeader.coilID) || thicknessHeader.CoilGeneratedId != previousThicknessHeader.CoilGeneratedId
                ...
                debug.printDebugMsg(tcp.setup.PLANTNAME, this.getClass().getCanonicalName(), " !!!!! A fejléc megváltozott : ==================================================="
                        + "\t sorszám:                  " + previousThicknessHeader.sorSzam + " ----> " + thicknessHeader.sorSzam
                        + "\t coilgenerated id:         " + previousThicknessHeader.CoilGeneratedId + " ----> " + thicknessHeader.CoilGeneratedId
                        + "\t coil id:                  " + previousThicknessHeader.coilID + " ----> " + thicknessHeader.coilID
                        + "\t setup thicness:           " + previousThicknessHeader.setupThickness + " ----> " + thicknessHeader.setupThickness
                        + "\t + thickness tolarence:    " + previousThicknessHeader.positivThicknessTolerance + " ----> " + thicknessHeader.positivThicknessTolerance
                        + "\t - thickness tolarence:    " + previousThicknessHeader.negativThicknessTolerance + " ----> " + thicknessHeader.negativThicknessTolerance
                        + "\t number of thickness data: " + previousThicknessHeader.numberOfThicknessData + " ----> " + thicknessHeader.numberOfThicknessData
                        + "\t tekercs vége jel        : " + previousThicknessHeader.endOfCoil + " ----> " + thicknessHeader.endOfCoil);
            ...
            if (thicknessHeader.CoilGeneratedId != previousThicknessHeader.CoilGeneratedId) {
                ...
                debug.printDebugMsg(tcp.setup.PLANTNAME, this.getClass().getCanonicalName(), "!!!!! Új tekercs Kezdődik --> thicknessHeader.CoilGeneratedId = " + thicknessHeader.CoilGeneratedId + " coilID=" + previousThicknessHeader.CoilGeneratedId);
    ...
    /*A vastagságmérő az utolsó adatra hibás értéket küld*/
    private void thicknessRecord(TCPConnectionServer tcp) {
        //Az utolsó adat eldobása vastagságmérő hibás jel miatt
        if (collectedReceiveTelegram.length > DELETEDTHICKNESSLENGTH) {
            collectedReceiveTelegram = Arrays.copyOf(collectedReceiveTelegram, collectedReceiveTelegram.length - DELETEDTHICKNESSLENGTH);
        }
        hhqcs.HHQCS.postgresql.thicknessRecord(thicknessHeader, Compressor.compress(collectedReceiveTelegram), tcp.setup);
    }
